---
title: Revive Hyperlocal
createdAt: 2026-04-20T11:00:00
categories: []
links: []
related: []
thread: microblog:2026/map-maker
# inReplyTo: microblog:2026/specific-post
---

Trying to make a simple tool to "revive" Hyperlocal. It should work in the browser without any signups or database. Someone should be able to open the page, map out a few points, lines, areas, and share the link.

I got a basic version working. Users can add points, lines, name them, give them colors, and find their addresses.

![Desktop map drawing tool with points, lines, and labeled shapes](/images/microblog/2026/revive-hyperlocal/desktop-map.webp)

![Mobile map drawing tool showing map annotations on a phone-sized screen](/images/microblog/2026/revive-hyperlocal/mobile-map.webp)
